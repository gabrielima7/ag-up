...
func main() {
	...
	flag.Usage = func() {
		_, _ = printer.Fprintf(os.Stderr, "\nUsage: ag-up [OPTIONS]\n\n")
		_, _ = printer.Fprintf(os.Stderr, "Google Antigravity Universal Updater (ag-up) %s\n", Version)
		_, _ = printer.Fprintf(os.Stderr, "Manages agy CLI, Antigravity IDE, and Antigravity Hub 2.0\n\n")
		_, _ = printer.Fprintf(os.Stderr, "Options:\n")
		flag.PrintDefaults()
		_, _ = printer.Fprintf(os.Stderr, "\nExamples:\n")
		_, _ = printer.Fprintf(os.Stderr, "  ag-up                    # Launch interactive menu\n")
		_, _ = printer.Fprintf(os.Stderr, "  ag-up --check            # Check versions (no download)\n")
		_, _ = printer.Fprintf(os.Stderr, "  ag-up --all              # Update everything concurrently\n")
		_, _ = printer.Fprintf(os.Stderr, "  ag-up --cli              # Update only the CLI\n")
		_, _ = printer.Fprintf(os.Stderr, "  ag-up --ide --retries 5  # Update IDE with 5 retries\n\n")
	}
	...
	if err := xdg.EnsureDirectories(allAppIDs); err != nil {
		_, _ = printer.Fprintf(os.Stderr, "ag-up: failed to create XDG directories: %v\n", err)
		os.Exit(1)
	}
	...
	m, err := manifest.Load()
	if err != nil {
		_, _ = printer.Fprintf(os.Stderr, "ag-up: failed to load manifest: %v\n", err)
		os.Exit(1)
	}
	...
	if err := ui.RunInteractiveMenu(ctx, m, maxRetries, Version); err != nil {
		...
		_, _ = printer.Fprintf(os.Stderr, "ag-up: %v\n", err)
		os.Exit(1)
	}
...
func runFlagMode(
	...
	if check {
		...
		if err != nil {
			_, _ = printer.Fprintf(os.Stderr, "ag-up: check failed: %v\n", err)
			return 1
		}
	...
	if len(specs) == 1 {
		...
		if err != nil {
			_, _ = printer.Fprintf(os.Stderr, "ag-up: update failed: %v\n", err)
			return 1
		}
	}
